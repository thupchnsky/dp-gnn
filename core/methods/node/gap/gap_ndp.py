...
class NodePrivGAP (GAP):
    ...
    def _aggregate(self, x: torch.Tensor, adj_t: SparseTensor) -> torch.Tensor:
        x = matmul(adj_t, x)
        # Note that the node to be replaced will be treated seperately. Hence, 
        # the effective sensitivity is 2*sqrt(max_degree)
        x = self.pma_mechanism(x, sensitivity=2*np.sqrt(self.max_degree))
        return x
    ...
    def configure_optimizer(self) -> DPOptimizer:
        ...
        optimizer = self.classifier_noisy_sgd.prepare_optimizer(optimizer, 2.0)
        return optimizer

    def configure_encoder_optimizer(self) -> DPOptimizer:
        optimizer = super().configure_encoder_optimizer()
        optimizer = self.encoder_noisy_sgd.prepare_optimizer(optimizer, 2.0)
        return optimizer
