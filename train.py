...
def run(seed:     Annotated[int,   ArgInfo(help='initial random seed')] = 12345,
        ...
        csbm_phi: Annotated[float,   ArgInfo(help='parameter phi for cSBM dataset generation')] = 0.0,
    ...
    with console.status('loading dataset'):
        ...
        data_initial = DatasetLoader(**loader_args).load(verbose=True, csbm_phi=csbm_phi)

    num_classes = data_initial.y.max().item() + 1
    num_features = data_initial.num_features
    ...
    if kwargs['method'].startswith('dpdgc'):
        method = Method(num_features=num_features, num_classes=num_classes, **method_args)
    else:
        method = Method(num_classes=num_classes, **method_args)
